...
const Item = (itemProps: ItemProps) => {
    ...
    const handleIsCompleted = (index: number) => {
        itemProps.changeIsCompleted(index, itemProps.id);
    }
    ...
    return (
        <div>
            <div className="m-auto mb-2 w-60 border-2 border-slate-500 rounded-lg p-2 text-lg flex justify-between">
                ...
                <div>
                    <button className="bg-green-300 p-1 rounded-full justify-end mr-2 text-white text-[40px]"
                        onClick={() => handleIsCompleted(itemProps.index)}
                    >V</button>
